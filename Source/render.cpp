		...
		if (!(level_cel_block & 0x8000))
		{
			...
			case 3: // upper (top transparent), with lighting
				...
				while (tmp_pbDst >= gpBufEnd)
				{
					...
					tmp_pbDst = &tmp_pbDst[xx_32 - WorkingWidth - 32];
					xx_32 -= 2;
					if (xx_32 < 0)
					{
						yy_32 = 2;
						do
						{
							...
							tmp_pbDst = &tmp_pbDst[yy_32 - WorkingWidth - 32];
							yy_32 += 2;
						} while (yy_32 != 32);
						return;
					}
	...
	LABEL_22:
		...
		case 8: // upper (solid), without lighting
			...
			do
			{
				...
				pdung_cels += 32;
				tmp_pbDst -= WorkingWidth; // why no -32?;;
				--i;
			} while (i);
			break;
	...
	if (!(level_cel_block & 0x8000))
	{
		...
		case 2: // lower (top transparent), with lighting
			...
			if (pbDst >= gpBufEnd)
			{
				...
				if (tile_42_45 > 45)
				{
					tmp_pbDst = (unsigned char *)(pbDst - WorkingWidth * 16);
					pdung_cels += 288;
				LABEL_98:
					yy_32 = 2;
					if (tmp_pbDst >= gpBufEnd)
		...
		if ((_BYTE)light_table_index == lightmax)
		{
			...
			case 3: // lower (bottom transparent), black
				...
				do
				{
					...
					tmp_pbDst = &tmp_pbDst[i - WorkingWidth - 32];
					i += 2;
				} while (i != 32);
				break;
		...
		if (!(level_cel_block & 0x8000))
		{
			...
			case 2: // lower (solid), with lighting
				...
				if (pbDst >= gpBufEnd)
				{
					...
					if (tile_42_45 > 45)
					{
						tmp_pbDst = pbDst - WorkingWidth * 16;
						pdung_cels += 288;
					LABEL_68:
						yy_32 = 2;
						if (tmp_pbDst >= gpBufEnd)
...
void __fastcall world_draw_black_tile(unsigned char *pbDst)
{
	...
	for (i = 1; ; ++i)
	{
		...
		tmp_pbDst = &tmp_pbDst[4 * i - (WorkingWidth + 64) + xx_32];
		if (!xx_32)
			break;
		xx_32 -= 2;
	}
	...
	do
	{
		tmp_pbDst += yy_32;
		memset(tmp_pbDst, 0, 4 * i);
		tmp_pbDst = &tmp_pbDst[4 * i - (WorkingWidth + 64) + yy_32];
		--i;
		yy_32 += 2;
	} while (yy_32 != 32);
}
