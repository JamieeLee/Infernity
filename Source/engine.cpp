...
void PrintDebugInfo() {
	if (GetAsyncKeyState(VK_TAB) & 0x8000) {
		std::stringstream ss;
		ss << "debuginfo: " << plr[myplr]._pNumInv << " " << plr[myplr].NumInvExpanded[0] << " " << plr[myplr].NumInvExpanded[1] << " " << plr[myplr].NumInvExpanded[2] << " " << plr[myplr].NumInvExpanded[3] << "GOLD1: " << plr[myplr]._pGold << " GOLD2: " << CalculateGold(myplr);;
...
void GenerateRareUniqueAffix(int i, int x, int y, int minlvl, int maxlvl, std::set<char>powers) {
	...
	if (sufPref == 0) {
		...
		} while (powers.find(PL_USuffix[sufidx2].PLPower) != powers.end());
