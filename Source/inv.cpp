...
void __cdecl DrawInv()
{
	...
	{
		InvDrawSlotBack(517 + GetWidthDiff(), 219, 2 * INV_SLOT_SIZE_PX, 2 * INV_SLOT_SIZE_PX);
		...
		if (pcursinvitem == INVITEM_HEAD)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_HEAD], true);
			if (!plr[myplr].InvBody[INVLOC_HEAD]._iStatFlag)
			{
				colour = 229;
			}
			CelDecodeClr(colour, 517 + GetWidthDiff(), 219, (char *)pCursCels, frame, frame_width, 0, 8);
		}

		if (plr[myplr].InvBody[INVLOC_HEAD]._iStatFlag)
		{
			CelDrawHdrOnly(517 + GetWidthDiff(), 219, (char *)pCursCels, frame, frame_width, 0, 8);
		}
		else
		{
			CelDrawHdrLightRed(517 + GetWidthDiff(), 219, (char *)pCursCels, frame, frame_width, 0, 8, 1);
		}
	}

	if (plr[myplr].InvBody[INVLOC_RING_LEFT]._itype != ITYPE_NONE)
	{
		InvDrawSlotBack(432 + GetWidthDiff(), 365, INV_SLOT_SIZE_PX, INV_SLOT_SIZE_PX);
		...
		if (pcursinvitem == INVITEM_RING_LEFT)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_RING_LEFT], true);
			if (!plr[myplr].InvBody[INVLOC_RING_LEFT]._iStatFlag)
			{
				colour = 229;
			}
			CelDecodeClr(colour, 432 + GetWidthDiff(), 365, (char *)pCursCels, frame, frame_width, 0, 8);
		}

		if (plr[myplr].InvBody[INVLOC_RING_LEFT]._iStatFlag)
		{
			CelDrawHdrOnly(432 + GetWidthDiff(), 365, (char *)pCursCels, frame, frame_width, 0, 8);
		}
		else
		{
			CelDrawHdrLightRed(432 + GetWidthDiff(), 365, (char *)pCursCels, frame, frame_width, 0, 8, 1);
		}
	}

	if (plr[myplr].InvBody[INVLOC_RING_RIGHT]._itype != ITYPE_NONE)
	{
		InvDrawSlotBack(633 + GetWidthDiff(), 365, INV_SLOT_SIZE_PX, INV_SLOT_SIZE_PX);
		...
		if (pcursinvitem == INVITEM_RING_RIGHT)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_RING_RIGHT], true);
			if (!plr[myplr].InvBody[INVLOC_RING_RIGHT]._iStatFlag)
			{
				colour = 229;
			}
			CelDecodeClr(colour, 633 + GetWidthDiff(), 365, (char *)pCursCels, frame, frame_width, 0, 8);
		}

		if (plr[myplr].InvBody[INVLOC_RING_RIGHT]._iStatFlag)
		{
			CelDrawHdrOnly(633 + GetWidthDiff(), 365, (char *)pCursCels, frame, frame_width, 0, 8);
		}
		else
		{
			CelDrawHdrLightRed(633 + GetWidthDiff(), 365, (char *)pCursCels, frame, frame_width, 0, 8, 1);
		}
	}

	if (plr[myplr].InvBody[INVLOC_AMULET]._itype != ITYPE_NONE)
	{
		InvDrawSlotBack(589 + GetWidthDiff(), 220, INV_SLOT_SIZE_PX, INV_SLOT_SIZE_PX);
		...
		if (pcursinvitem == INVITEM_AMULET)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_AMULET], true);
			if (!plr[myplr].InvBody[INVLOC_AMULET]._iStatFlag)
			{
				colour = 229;
			}
			CelDecodeClr(colour, 589 + GetWidthDiff(), 220, (char *)pCursCels, frame, frame_width, 0, 8);
		}

		if (plr[myplr].InvBody[INVLOC_AMULET]._iStatFlag)
		{
			CelDrawHdrOnly(589 + GetWidthDiff(), 220, (char *)pCursCels, frame, frame_width, 0, 8);
		}
		else
		{
			CelDrawHdrLightRed(589 + GetWidthDiff(), 220, (char *)pCursCels, frame, frame_width, 0, 8, 1);
		}
	}

	if (plr[myplr].InvBody[INVLOC_HAND_LEFT]._itype != ITYPE_NONE)
	{
		InvDrawSlotBack(401 + GetWidthDiff(), 320, 2 * INV_SLOT_SIZE_PX, 3 * INV_SLOT_SIZE_PX);
		...
		if (pcursinvitem == INVITEM_HAND_LEFT)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_HAND_LEFT], true);
			if (!plr[myplr].InvBody[INVLOC_HAND_LEFT]._iStatFlag)
			{
				colour = 229;
			}
			CelDecodeClr(colour, screen_x + GetWidthDiff(), screen_y, (char *)pCursCels, frame, frame_width, 0, 8);
		}

		if (plr[myplr].InvBody[INVLOC_HAND_LEFT]._iStatFlag)
		{
			CelDrawHdrOnly(screen_x + GetWidthDiff(), screen_y, (char *)pCursCels, frame, frame_width, 0, 8);
		}
		else
		{
			CelDrawHdrLightRed(screen_x + GetWidthDiff(), screen_y, (char *)pCursCels, frame, frame_width, 0, 8, 1);
		}

		if (plr[myplr].InvBody[INVLOC_HAND_LEFT]._iLoc == ILOC_TWOHAND)
		{
			InvDrawSlotBack(631 + GetWidthDiff(), 320, 2 * INV_SLOT_SIZE_PX, 3 * INV_SLOT_SIZE_PX);
			light_table_index = 0;
			cel_transparency_active = 1;

			CelDecodeHdrLightTrans(
				frame_width == INV_SLOT_SIZE_PX
				? &gpBuffer->row[160].pixels[581 + GetWidthDiff()]
				: &gpBuffer->row[160].pixels[567 + GetWidthDiff()],
	...
	if (plr[myplr].InvBody[INVLOC_HAND_RIGHT]._itype != ITYPE_NONE)
	{
		...
		if (pcursinvitem == INVITEM_HAND_RIGHT)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_HAND_RIGHT], true);
	...
	{
		InvDrawSlotBack(517 + GetWidthDiff(), 320, 2 * INV_SLOT_SIZE_PX, 3 * INV_SLOT_SIZE_PX);
		...
		if (pcursinvitem == INVITEM_CHEST)
		{
			...
			colour = GetItemHighlightColor(0, &plr[myplr].InvBody[INVLOC_CHEST], true);
			if (!plr[myplr].InvBody[INVLOC_CHEST]._iStatFlag)
			{
				colour = 229;
			}
			CelDecodeClr(colour, 517 + GetWidthDiff(), 320, (char *)pCursCels, frame, frame_width, 0, 8);
		}

		if (plr[myplr].InvBody[INVLOC_CHEST]._iStatFlag)
		{
			CelDrawHdrOnly(517 + GetWidthDiff(), 320, (char *)pCursCels, frame, frame_width, 0, 8);
		}
		else
		{
			CelDrawHdrLightRed(517 + GetWidthDiff(), 320, (char *)pCursCels, frame, frame_width, 0, 8, 1);
		}
	}
	...
	for (int j = 0; j < NUM_INV_GRID_ELEM; j++)
	{
		if (plr[myplr].InvGrid[j] > 0) // first slot of an item
		{
			...
			if (pcursinvitem == ii + INVITEM_INV_FIRST)
			{
				...
				colour = GetItemHighlightColor(0, &plr[myplr].InvList[ii], true);
...
LABEL_191:
				...
				case ILOC_BELT:
					...
					if ( plr[v3].HoldItem._itype != ITYPE_GOLD )
					{
						...
						{
							qmemcpy((char *)plr[0].SpdList + v53, &plr[v3].HoldItem, StructSize<ItemStruct>());
						}
						else
						{
...
void __fastcall CheckInvSwap(int pnum, BYTE bLoc, int idx, WORD wCI, int seed, BOOL bId, int rare)
{
...
int __cdecl CheckInvHLight()
{
	...
	if ( v0 < 13 || v0 > 18 )
	{
		...
		if ( v0 < 25 || v0 > 64 )
		{
			...
			v5 = v0 - 65;
			drawsbarflag = 1;
			result = StructSize<ItemStruct>() * v5;
			v2 = &v3->SpdList[v5];
