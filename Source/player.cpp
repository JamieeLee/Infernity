...
bool CanRun(int pnum) {
	return (!currlevel && GetConfigBoolVariable("runInTown")) || (currlevel && GetConfigBoolVariable("runInDungeon") && MonstersInCombat(pnum) == 0);
}
...
void __fastcall CreatePlayer(int pnum, char c)
{
	...
	plr[pnum].version = CurVersion;
	plr[pnum].currentInventoryIndex = 0;


	memset(plr[pnum].InvGridExpanded, 0, sizeof(char) * 40);
	memset(plr[pnum].InvListExpanded, 0, sizeof(ItemStruct) * 40);
	for (int i = 0; i < 4; ++i) {
		plr[pnum].NumInvExpanded[i] = 0;
		for (int j = 0; j < 40; ++j) {
			plr[pnum].InvListExpanded[i][j]._itype = -1;
			plr[pnum].InvGridExpanded[i][j] = 0;
		}
	}
	plr[pnum].NumInvExpanded[0] = 2;
	memset(&plr[pnum].alternateWeapons[0], 0, StructSize<ItemStruct>());
	memset(&plr[pnum].alternateWeapons[1], 0, StructSize<ItemStruct>());
	...
	for (int i = 0; i < MAX_STASH_TABS; ++i) {
		plr[pnum].StashNumInv[i] = 0;
		for (int j = 0; j < 40; ++j) {
			plr[pnum].StashInvList[i][j]._itype = -1;
			plr[pnum].StashInvGrid[i][j] = 0;
		}
		strcpy(&plr[pnum].StashNames[i][0], "Click to rename");
	}
	plr[pnum].lastTab = 0;
...
void __fastcall AddPlrMonstExper(int lvl, int exp, char pmask, int x, int y)
{
	...
	if (true) {// totplrs && (1 << myplr) & pmask) {
		AddPlrExperience(myplr, lvl, exp /*/ totplrs*/,x,y); // enabling full xp gain for all players
	}
}
...
void __fastcall PM_ChangeOffset(int pnum)
{
	...
	int mul = CanRun(pnum) ? 2 : 1; // Player run
...
void __fastcall ClrPlrPath(int pnum)
{
	...
	memset(plr[pnum].walkpath2, -1, sizeof(plr[pnum].walkpath2));
}
